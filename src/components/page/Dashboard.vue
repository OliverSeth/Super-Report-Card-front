...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            // }
            let api=this.$api.userApi.findExam;
            api.data={
                TEACHER_ID:id
            };
            this.axios(api).then(response=>{
                ...
                for(let i=0;i<data.length;i++){
                    if(data[i].days_LEFT>=0){
                        let obj={};
                        obj.title=data[i].exam_DATE+space+data[i].class_NAME+data[i].exam_NAME+space+'还剩 '+data[i].days_LEFT+' 天';
                        this.examList.push(obj);
                    }
                }
            });
            let api2=this.$api.userApi.getCourse;
            api2.data={
                TEACHER_ID:id
            };
